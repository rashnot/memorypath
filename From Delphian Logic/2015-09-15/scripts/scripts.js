...
var paramWhatsNew = {
	count:40,
	cutOffDays:-30, // integer with (-) or ""
	cutOffDate:"09/08/2015", // MM/DD/YYYY or ""
...
var arrAutoJumpSec = ["li", "lo", "le", "lc", "zh", "cl"];
var strAutoJumpSec = getParameterByName('sec');
var numAutoJumpSec = $.inArray(strAutoJumpSec.toLowerCase(), arrAutoJumpSec);

var strAutoJumpAcademy = getParameterByName('aid');
...
function fBuildBtnEvents(){
	...
	$('#smallImage').click(function(event) {
		$(".btnWhatsNew").removeClass("selected").addClass("normal");	
		$('#bigImage').fadeIn(100)
	...
	if (numAutoJumpSec != -1){
		setTimeout(function(){
			$('#imageholder .btn:eq('+numAutoJumpSec+')').trigger( "click" );
			numAutoJumpSec = -1;
		}, 500);
	}
...
function fGetAllCourses(){
	if($().SPServices){
		...
		viewFields += '<FieldRef Name="DestinationLink"/><FieldRef Name="LinkDescription"/>';
		viewFields += '<FieldRef Name="Duration"/><FieldRef Name="hours_x002f_days"/>';
		...
		$().SPServices({
		   ...
		   completefunc: function (xData, Status) {
				...
				$(xData.responseXML).find("z\\:row, row").each(function (idx) {
					...
					if(paramWhatsNew.sortBy == "Modified"){
						tmpObj.cDate = myrow.attr("ows_Modified");
					}else{
						tmpObj.cDate = myrow.attr("ows_Created");
					}
				...
				if(!bWhatsNewDataCourse){
					for (var i=0;i<allCourseData.length;i++){
						if(fn_DateCompare(allCourseData[i].cDate) == 1 ){
							WhatsNewData.push(allCourseData[i])
						}
					} 
					bWhatsNewDataCourse =  true;
				}
			}
		});			
	}
	...
	if($().SPServices){
		...
		$().SPServices({
		   ...
		   completefunc: function (xData, Status) {
				...
				if(!bWhatsNewDataCinemas){
					for (var i=0;i<allCinemaData.length;i++){
						if(fn_DateCompare(allCinemaData[i].cDate) == 1 ){
							WhatsNewData.push(allCinemaData[i])
						}
					} 
					bWhatsNewDataCinemas =  true;
				}				
			}
		});			
	}
	...
	if($().SPServices){
		...
		$().SPServices({
		   ...
		   completefunc: function (xData, Status) {
				...
				if(!bEWhatsNewDataxpertise){
					for (var i=0;i<allExpertiseData.length;i++){
						if(fn_DateCompare(allExpertiseData[i].cDate) == 1 ){
							WhatsNewData.push(allExpertiseData[i])
						}
					} 
					bEWhatsNewDataxpertise =  true;
				}					
			}
		});			
	}
...
function fBuildAcademyTiles(){
	...
	if(CourseType=="Internal")
		str += '<p style="text-align:left; color:#666;">Explore our open learning community. The academies, functions, and countries open up different learning resources so you can access &ndash; from this single convenient location - an even wider range of learning (including My Learning). In addition to getting access to material that will help you improve in your current roles, you have a broader choice of  tools and services that will help you develop skills needed for roles to which you may aspire.</p><p style="text-align:left; color:#666;">This catalog equips you to drive your continuous learning to achieve our strategy. The open learning community concept makes sharing knowledge easier and helps everyone learn from each other.</p><p style="text-align:left; color:#666;">Access to &#39;My Learning&#39; on the Group Learning Management System (GEMS) or to your local learning site have not changed. Those who wish to access them directly can continue to do so.</p>';
	...
	for(var i=0; i<noOfRows; i++){
		...
		for(var j=0; j<noOfColumns; j++){
			if(academyData[academyIndex].AcademyType != "Country"){
				...
				if(fFindInWhatsNew(academyData[academyIndex].ID, "academy")) str += '<div class="whatsNew"></div>';
				str += '</div></td>';
			} else {
				tdAdded++;
				str += '<td><div id="Academy_'+academyData[academyIndex].ID+'" class="academy-tile academy-tile-normal" title="'+academyData[academyIndex].Description+'">\
	...
	if (strAutoJumpAcademy != ""){
		if ($('#Academy_'+strAutoJumpAcademy).length > 0){
		
			setTimeout(function(){
				
				$('#Academy_'+strAutoJumpAcademy).trigger( "click" );
				
				if ($('#s4-workspace').length > 0 ){
					$('#s4-workspace').animate({scrollTop: $('#Academy_'+strAutoJumpAcademy).offset().top - 50},'slow');
				}
	...
	if(courseData.length > 0){
		...
		for(var i=0; i<courseData.length; i++){
			str += '<td id="CourseListItem_'+courseData[i].ID+'" class="CourseListItem">';
				str += '<div class="courseImage" onClick="fShowCourseDetails('+courseData[i].ID+', this)"><div>'+courseData[i].PublishingPageImage+'</div>';
				if(fFindInWhatsNew(courseData[i].ID)) str += '<div class="whatsNew"></div>';
				str += '</div>';
	...
	if(cinemaData.length > 0){
		...
		for(var i=0; i<cinemaData.length; i++){
			str += '<td id="CourseListItem_'+cinemaData[i].ID+'" class="CourseListItem">';
				str += '<div class="courseImage" onClick="fShowCinemaDetails('+cinemaData[i].ID+', this)">';
				str += '<div>'+cinemaData[i].CourseImage+'</div>';
				if(fFindInWhatsNew(cinemaData[i].ID)) str += '<div class="whatsNew"></div>';
				str += '</div>';
				str += '<div class="seperator" style="width:100px;clear:both;"></div>';
	...
	if(expertiseData.length > 0){
		...
		for(var i=0; i<expertiseData.length; i++){
			str += '<td id="CourseListItem_'+expertiseData[i].ID+'" class="CourseListItem">';
				str += '<div class="courseImage" onClick="fShowExpertiseDetails('+expertiseData[i].ID+', this)"><div>'+expertiseData[i].CourseImage+'</div>';
				if(fFindInWhatsNew(expertiseData[i].ID)) str += '<div class="whatsNew"></div>';
				str += '</div>';
				str += '<div class="seperator" style="width:100px;clear:both;"></div>';
...
function fShowCourseDetails(ID, obj, type, academy){
	if(!type){
		cType = CourseType;
		var courseObj = getListItem(courseData, 'ID', ID);
	}else{
		cType = type;
		var courseObj = getItemInWhatsNew(ID, cType);
	}
//WT global
        WTCourseName = fWTgetCourseTitle(cType, ID);
	...
	*/
	...
	str += '<div class="seperator clearF"></div>';	
	if(fCheckLearningPlanList(courseObj.ID, cType)){
		str += '<div class="addToPlanBtn floatR btnDisabled">Added to My Learning Cart</div>';	
	}else{
		if(!type){
			str += '<div id="add_Link_Below_'+courseObj.ID+'" class="CourseLink floatR" onClick="fAddToPlan(\''+courseObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}else{
			str += '<div id="add_Link_Below_'+courseObj.ID+'" class="CourseLink floatR" onClick="fAddToPlanFromWhatsNew(this, \''+courseObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}
	...
	if(!type){
		...
		next_tr.find('.whatsnew-tile-details-inner').html('').html(str).show();
	}
...
function fShowCinemaDetails(ID, obj, type, academy){
	if(!type){
		cType = CourseType;
		var cinemaObj = getListItem(cinemaData, 'ID', ID);
	}else{
		cType = type;
		var cinemaObj = getItemInWhatsNew(ID, cType);
	}
//WT global
        WTCourseName = fWTgetCourseTitle(cType, ID);
        //Webtrends start todo
	...
	if(fCheckLearningPlanList(cinemaObj.ID, cType)){
		str += '<div class="addToPlanBtn floatR btnDisabled">Added to My Learning Cart</div>';	
	}else{
		if(!type){
			str += '<div id="add_Link_Below_'+cinemaObj.ID+'" class="CourseLink floatR" onClick="fAddToPlan(\''+cinemaObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}else{
			str += '<div id="add_Link_Below_'+cinemaObj.ID+'" class="CourseLink floatR" onClick="fAddToPlanFromWhatsNew(this, \''+cinemaObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}
		str += '<div class="clearF" style="height:3px;"></div>';
	}
	str += '<div class="seperator clearF"></div>';
...
function fShowExpertiseDetails(ID, obj, type, academy){
	if(!type){
		cType = CourseType;
		var expertiseObj = getListItem(expertiseData, 'ID', ID);
	}else{
		cType = type;
		var expertiseObj = getItemInWhatsNew(ID, cType);
	}
//WT global
        WTCourseName = fWTgetCourseTitle(cType, ID);
        //Webtrends start todo
	...
	if(fCheckLearningPlanList(expertiseObj.ID, cType)){
		str += '<div class="addToPlanBtn floatR btnDisabled">Added to My Learning Cart</div>';	
	}else{
		if(!type){
			str += '<div id="add_Link_Below_'+expertiseObj.ID+'" class="CourseLink floatR" onClick="fAddToPlan(\''+expertiseObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}else{
			str += '<div id="add_Link_Below_'+expertiseObj.ID+'" class="CourseLink floatR" onClick="fAddToPlanFromWhatsNew(this, \''+expertiseObj.ID+'\',\''+cType+'\')" >Add to My Learning Cart >></div>';	
		}
		str += '<div class="clearF" style="height:3px;"></div>';
	}
	str += '<div class="seperator clearF"></div>';
...
function fAddToPlan(ID, type, academy){
	if(!type) {
		cType = CourseType;
	}else{
		cType = type;
	}
	if(academy) WTuriAcademy = academy;
	//Webtrends start
	WTCourseName = fWTgetCourseTitle(cType, ID);
	...
	listToBeUpdated += '<Field Name="Title"><![CDATA[' + cType + ']]></Field>';
...
function fCheckLearningPlanList(ID, cType){
	if(!cType) cType = CourseType;
	var isRecordFound = false;
	for (var i=0;i<userLearningCartData.length;i++){
		if ((userLearningCartData[i].Title == cType) && userLearningCartData[i].RowID == ID){
			isRecordFound = true;
			break;
		}
	}
...
function fWTgetCourseTitle(thisCourseType, thisCourseID){
	for(var i=0; i<WhatsNewData.length; i++){
		if(WhatsNewData[i].ID==thisCourseID){return WhatsNewData[i].Title;}
	}
...
var WhatsNewIndex = 0;
var WhatsNewData = [];
var bWhatsNewDataCourse = false;
var bWhatsNewDataCinemas = false;
var bEWhatsNewDataxpertise = false;

function fGetWhatsNew(obj){
	if($(obj).hasClass("selected")) return;
	fCollapseMenu();
	$(obj).removeClass("normal").addClass("selected");
	setTimeout(function(){	
		fGetLearningCartData();
		if(!bWhatsNewDataCourse){ fGetAllCourses(); }
		if(!bWhatsNewDataCinemas){ fGetAllCinemas(); }
		if(!bEWhatsNewDataxpertise){ fGetAllExpertise(); }

		WhatsNewData.sort(function(a, b){
			var lTempArr = a.cDate.split(" ")[0].split("-")
			a = lTempArr[2] + "/" + lTempArr[1] + "/" + lTempArr[0]
			lTempArr = b.cDate.split(" ")[0].split("-")
			b = lTempArr[2] + "/" + lTempArr[1] + "/" + lTempArr[0]		
			var dateA=new Date(a), dateB=new Date(b)
			return dateB-dateA //sort by date ascending
		})
		$('#page-content').show();
		fLoadPageWhatsNew(obj);
	}, 500);	
	
	/*for (var i=0;i<WhatsNewData.length;i++){
		alert(WhatsNewData[i].cDate)
	}*/		
}

function fLoadPageWhatsNew(obj){
	//Webtrends start
	WTuriMainMenu = WTSITE + "What's New";
	//dcsMultiTrack('DCS.dcsuri', WTuriMainMenu, 'WT.es', WTuriMainMenu, 'WT.ti', 'MainMenu' + n, 'WT.dl', '59');
        //Webtrends end
	
	$('#page-content #topicImage').removeClass().addClass('topicImageWhatsNew');
	$('#page-content .topicHeading').html('').html($(obj).attr('title'));	
	fBuildWhatsNew();
	$('#transitionPatch').animate({'opacity':0},500,function(){$(this).hide();})
}


function fBuildWhatsNew() {
	var str = '';
	$('#page-content .content').html('')

	str += '<p style="text-align:left; color:#666;">Description...</p>';
	str += '<table class="whatsNew-grid">';

	var WhatsNewCount = WhatsNewData.length;
	if(WhatsNewCount > paramWhatsNew.count) WhatsNewCount = paramWhatsNew.count;
	var colNo = 4;
	var rowNo = Math.ceil(WhatsNewCount/colNo);	
	WhatsNewIndex =0;
	
	for(var i=0; i<rowNo; i++){
		var _class = ''
		if (rowNo > 1){
			if(i == rowNo-1) _class = 'lastRow';
		}
		str += '<tr class="'+_class+'">';
		...
		for(var j=0; j<colNo; j++){
			tdAdded++;
			str += '<td class="whatsnewTD normal"><div id="CourseListItem_'+WhatsNewData[WhatsNewIndex].ID+'" class="whatsnew-tile whatsnew-tile-normal CourseListItem">';
				//str += '<div class="title"><b>'+WhatsNewData[WhatsNewIndex].Title+'</b></div>';
				if(WhatsNewData[WhatsNewIndex].PublishingPageImage){
					str += '<div class="courseImage" onClick="fShowWhatsNewDetails(this, '+WhatsNewData[WhatsNewIndex].ID+', \''+WhatsNewData[WhatsNewIndex].CourseType+'\')"><div>'+WhatsNewData[WhatsNewIndex].PublishingPageImage+'</div></div>';
				}
				if(WhatsNewData[WhatsNewIndex].CourseImage){
					str += '<div class="courseImage" onClick="fShowWhatsNewDetails(this, '+WhatsNewData[WhatsNewIndex].ID+', \''+WhatsNewData[WhatsNewIndex].CourseType+'\')"><div>'+WhatsNewData[WhatsNewIndex].CourseImage+'</div></div>';
				}				
				str += '<div class="description">'+WhatsNewData[WhatsNewIndex].Title+'</div>';
				if (WhatsNewData[WhatsNewIndex].Level){
					str += '<div class="Level"><b>'+WhatsNewData[WhatsNewIndex].Level+'</b></div>';
				}
				str += '<div class="adademyName"><b>'+WhatsNewData[WhatsNewIndex].Academy.split("#")[1]+'</b></div>';
			str += '</div>';
			str += '<div class="date">'+fFormatDate2(WhatsNewData[WhatsNewIndex].cDate)+'</div>';
			str += '<div class="btns">';
				str += '<div class="btn viewDetailBtn" title="View Details" onClick="fShowWhatsNewDetails(this, '+WhatsNewData[WhatsNewIndex].ID+', \''+WhatsNewData[WhatsNewIndex].CourseType+'\')"></div>';
				if(fCheckLearningPlanList(WhatsNewData[WhatsNewIndex].ID, WhatsNewData[WhatsNewIndex].CourseType)){
					str += '<div id="" title="Added to My Learning Cart" class="btn addToPlanBtn btnDisabled"></div>';
				}else{
					str += '<div id="addToPlanBtn_'+WhatsNewData[WhatsNewIndex].ID+'" title="Add to My Learning Cart" class="btn addToPlanBtn addToPlanBtnActive" onClick="fAddToPlanFromWhatsNew(this, '+WhatsNewData[WhatsNewIndex].ID+', \''+WhatsNewData[WhatsNewIndex].CourseType+'\')"></div>';
				}
				var lLinkNum = fAddToGLinks(WhatsNewData[WhatsNewIndex].DestinationLink)
				str += '<div class="btn gotoLinkBtn" onClick="fOpenLinkFromGLinks('+lLinkNum+')"></div>';
				str += '<div style="clear:both;"></div>';
			str += '</div>';
			str += '</td>';
			WhatsNewIndex++;
			if(WhatsNewIndex == WhatsNewCount){
				break;
			}	
		}	
		...
		str += '</tr>';
		str += '<tr class="trTileDetails">';
			str += '<td class="tdTileDetails" colspan='+colNo+'>';
				str += '<div class="whatsnew-tile-details"><div id="AcademyCourseDetails" class="whatsnew-tile-details-inner"></div>';
			str += '</td>';
		str += '</tr>';		
	}
	str += '</table>';
	...
	$( ".whatsnew-tile img" ).load(function() {
		ResizeBigPhotoRatio($(this)[ 0 ],100,75);
	});
}

function fShowWhatsNewDetails(obj, id, type, AcademyID){
	
	WTAcademyName = getItemInWhatsNew(id, type).Academy.split("#")[1];
	var parentTile = $(obj).closest(".whatsnewTD");
	var parentTR = $(obj).closest("tr");
	
	if(parentTile.hasClass("current")) return;
	$(".whatsnewTD").removeClass("current").addClass("normal")
	parentTile.removeClass("normal").addClass("current")
	
	var _tIndex = parentTR.find('.whatsnewTD').index(parentTile);
	
	next_tr = parentTR.next();
	var bgPos = ((_tIndex*154)+68)+'px 0px';
	next_tr.find('td').removeClass().addClass('tdTileDetailsDown').css('backgroundPosition', bgPos)	
	if(parentTR.hasClass('lastRow')) {
		next_tr = parentTR.prev();
		bgPos = ((_tIndex*154)+68)+'px bottom';
		next_tr.find('td').removeClass().addClass('tdTileDetailsUp').css('backgroundPosition', bgPos)	
	}
	
	var expanded_tr = $(".whatsNew-grid").find( ".expanded" );	
	if(expanded_tr.length > 0 && ($(".whatsNew-grid").find( ".trTileDetails" ).index(next_tr) != $(".whatsNew-grid").find( ".trTileDetails" ).index(expanded_tr))){
		expanded_tr.find('.whatsnew-tile-details .whatsnew-tile-details-inner').html('')
		expanded_tr.find(".whatsnew-tile-details").animate({
			height: 0
		},500, function(){
			expanded_tr.removeClass('expanded');
			expanded_tr.hide();
		});		
	}	

	next_tr.show();
	next_tr.find(".whatsnew-tile-details").height(next_tr.find(".whatsnew-tile-details-inner").height());

	WTuriAcademy = WTuriMainMenu + '/' + WTAcademyName; //+ '(' + academyID + ')';
	dcsMultiTrack('DCS.dcsuri', WTuriAcademy, 'WT.es', WTuriAcademy, 'WT.ti', WTAcademyName, 'WT.dl', '59');

	if(type == "Internal" || type == "External" ) fShowCourseDetails(id, obj, type, WTAcademyName)
	if(type == "Cinema") fShowCinemaDetails(id, obj, type, WTAcademyName)
	if(type == "Expertise") fShowExpertiseDetails(id, obj, type, WTAcademyName)

	var newHeight = next_tr.find(".whatsnew-tile-details-inner").height()+50;
	next_tr.find(".whatsnew-tile-details").animate({
		height: newHeight
	}, 500, function(){next_tr.addClass('expanded')});
	
}

function fAddToPlanFromWhatsNew(obj, id , type, academy){
	AcademyName = getItemInWhatsNew(id, type).Academy.split("#")[1];
	
	var parentTile = $(obj).closest(".whatsnewTD");

	if($(obj).attr('class').indexOf('btnDisabled') != -1) return;
    
	if($(obj).hasClass("CourseLink")){  
		$(obj).addClass('btnDisabled').html('Added to My Learning Cart')
		$(".whatsNew-grid .current .addToPlanBtn").removeClass('addToPlanBtnActive').addClass('btnDisabled').attr('title','Added to My Learning Cart')
	}else{
		$(obj).removeClass('addToPlanBtnActive').addClass('btnDisabled').attr('title','Added to My Learning Cart')
	}		
	if(parentTile.hasClass("current")){
		$('#AcademyCourseDetails .addToPlanBtn').addClass('btnDisabled').html('Added to My Learning Cart')
	}
	
	fAddToPlan(id , type, AcademyName)
}

function fFormatDate2(lref){
	var lTempArr = lref.split(" ")[0].split("-")
	return DateA = lTempArr[2] + "/" + lTempArr[1] + "/" + lTempArr[0]
}

function fn_DateCompare(DateA) {
	if(paramWhatsNew.cutOffDate == "" && paramWhatsNew.cutOffDays == "") return 0;
	
	var lTempArr = DateA.split(" ")[0].split("-")
	DateA = lTempArr[2] + "/" + lTempArr[1] + "/" + lTempArr[0]
	var a = new Date(DateA);
   
	if(paramWhatsNew.cutOffDate != ""){
		var DateB = paramWhatsNew.cutOffDate;  
		var b = new Date(DateB);	
	}
	if(paramWhatsNew.cutOffDays != ""){
		var c = Date.today().add(paramWhatsNew.cutOffDays).days();	
	}
	if(paramWhatsNew.cutOffDate != "" && paramWhatsNew.cutOffDays != ""){
		if((Date.compare(a, b) == 0 || Date.compare(a, b) == 1) && (Date.compare(a, c) == 0 || Date.compare(a, c) == 1)){
			return 1;
		}
	}else if(paramWhatsNew.cutOffDate != ""){
		if(Date.compare(a, b) == 0 || Date.compare(a, b) == 1){
			return 1;
		}
	}else{
		if(Date.compare(a, c) == 0 || Date.compare(a, c) == 1){
			return 1;
		}	
	}
...
function fFindInWhatsNew(id, type){
	for (var i=0;i<WhatsNewData.length;i++){
		if(type == "academy"){
			if (WhatsNewData[i].AcademyID == id && CourseType == WhatsNewData[i].CourseType){
				return true;
			}
		}else{
			if (WhatsNewData[i].ID == id && CourseType == WhatsNewData[i].CourseType){
				return true;
			}		
		}
	}	
...
function getItemInWhatsNew(id, CourseType){
	for (var i=0;i<WhatsNewData.length;i++){
		if (WhatsNewData[i].ID == id && CourseType == WhatsNewData[i].CourseType){
			return WhatsNewData[i];
		}		
	}	
}

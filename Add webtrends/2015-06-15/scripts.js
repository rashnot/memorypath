// Create dummy dcsmultitrack if no webtrends present
...
function fLoadPageContent(n) {
    ...
    if (currentContentIndex == LEARNING_CATALOGUE || currentContentIndex == LEARNING_FROM_OUTSIDE || currentContentIndex == LEARNING_FROM_EXPERTISE || currentContentIndex == LEARNING_FROM_CINEMA) {
        if (currentContentIndex == LEARNING_CATALOGUE)
            CourseType = "Internal";
        if (currentContentIndex == LEARNING_FROM_OUTSIDE)
            CourseType = "External";
        if (currentContentIndex == LEARNING_FROM_EXPERTISE)
            CourseType = "Expertise";
    ...
    $('#transitionPatch').animate({'opacity': 0}, 500, function () {
        ...
        if (_index < 0)
            _index = 7;
        if ('object' === typeof $('.btnBack').data('qtip'))
            $('.btnBack').qtip("destroy");
        $('.btnBack').qtip({content: $('.btnImage').eq(_index).attr('titleText')})
        _index = currentContentIndex + 1;
        if (_index > 7)
            _index = 0;
        if ('object' === typeof $('.btnNext').data('qtip'))
            $('.btnNext').qtip("destroy");
...
function fBuildLearningFromHistory() {
    ...
    str += 'Be inspired by how Zurich has evolved from a Swiss insurer into one of the world’s leading insurance companies.<br><br><a href="https://collaboration.zurich.com/community/00935/01/default.aspx" target="_blank">View and Learn &raquo;</a></div>'
...
function fBuildLearningInsuranceZurichWay() {
    ...
    var str = '';
    $('#page-content .content').html('')
    str = '<p style="text-align:center;  color:#666;">Zurich Business Glossary paragraph<br/><a href="https://businessglossary.zurich.com:9445/ibm/iis/igc/" target="_blank">Zurich Business Glossary</a></p>'
    str += '<p style="text-align:center;  color:#666;">Acronyms paragraph<br/><a href="https://intranet.zurich.com/content/Pages/Jargonbuster.aspx" target="_blank">Acronyms</a></p>'
    $('#page-content .content').html(str)
        ...
        $().SPServices({
            ...
            completefunc: function (xData, Status) {
                ...
                $(xData.responseXML).find("z\\:row, row").each(function (idx) {
                    ...
                    if (CommunityURL == undefined || CommunityURL == 'undefined')
                        CommunityURL = ''
                    else
                    ...
                    if (LearningCinemaURL == undefined || LearningCinemaURL == 'undefined')
                        LearningCinemaURL = ''
                    else
                        LearningCinemaURL = LearningCinemaURL.split(",")[0]
...
function fAddToPlan(ID) {
    ...
    //Webtrends end
    if ($('#addToPlanBtn_' + ID).attr('class').indexOf('btnDisabled') != -1)
        return;
    if ($('#AcademyCourseDetails .addToPlanBtn').length > 0) {
        if ($('#AcademyCourseDetails .addToPlanBtn').attr('class').indexOf('btnDisabled') != -1)
            return;
    }
...
function fOpenLink(_url) {
    //Webtrends start
    if (typeof WTuriAcademy == 'undefined') {
        WTuriAcademy = 'RHL';
        WTCourseName = '';
    }
...
function fWTgetCourseTitle(thisCourseType, thisCourseID) {
    if (thisCourseType == "Internal" || thisCourseType == "External") {
        ...
        for (var i = 0; i < allCinemaData.length; i++) {
            if (allCinemaData[i].ID == thisCourseID) {
                return allCinemaData[i].Title;
            }
        }
    }
        ...
        for (var i = 0; i < allExpertiseData.length; i++) {
            if (allExpertiseData[i].ID == thisCourseID) {
                return allExpertiseData[i].Title;
            }
        }
    }
...
function fWTanalyseURL(_url) {
    ...
    if (WTURL.indexOf("intranet.zurich.com") > -1) {
        WTclassify = "GtoZ"; //Z-Net
    }
    if (WTURL.indexOf("collaboration.zurich.com") > -1) {
        WTclassify = "GtoC"; //Collab site/page
    }
    if (WTURL.indexOf("glms") > -1) {
        WTclassify = "GtoL"; //My Learning
    }
    if (WTURL.indexOf("cinemaflexi") > -1) {
        WTclassify = "GtoV"; //Video
    }
    if (WTURL.indexOf("globuz.zurich.com") > -1) {
        WTclassify = "GtoG"; //Globuz
    }
    if (WTURL.indexOf("tzw.zurich.com") > -1) {
        WTclassify = "GtoT"; //TZW
    }
    if (WTURL.indexOf("www.yammer.com") > -1) {
        WTclassify = "GtoY"; //Yammer
    }
    return WTclassify;
}
